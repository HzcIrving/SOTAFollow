...
1. **无时序信息**：仅单帧推理，速度估计不准、定位不稳定。
2. **3D PE 数据无关**：仅由固定网格 3D 坐标生成，与图像内容无关。
3. **单任务**：只支持 3D 检测，无法做分割、车道线。
4. **鲁棒性不足**：对相机外参噪声、传感器丢帧敏感。

## 1.3 本文动机

1. 用 **3D PE 做时序对齐**，替代 BEV 空间显式特征变换。
2. 让 3D PE 变成 **数据驱动（data-dependent）**。
3. 用 **稀疏查询**统一多任务，避免稠密 BEV query 效率低。
4. 提供全面鲁棒性分析。
...
## 3.2 时序建模：3D Coordinates Alignment（核心）
