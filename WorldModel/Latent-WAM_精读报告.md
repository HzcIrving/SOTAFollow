...
**输入**：时序多视角图像 $I \in ℝ^{T \times M \times H \times W \times 3}$

1. 图像嵌入为 patch tokens：$X \in ℝ^{T \times M \times S \times D_e}$
   - $T$：时序帧数
   - $M$：相机数量
   - $S$：每张图的 patch 数
   - $D_e = 768$

2. 随机初始化场景查询 $Q_{\text{scene}} \in ℝ^{T \times M \times N \times D_e}$，与 $X$ 拼接后送入 DINO encoder $ℰ$（含 MLP 投影到 $D_l = 256$ 维潜在空间）：

$$
\hat{Q}_{\text{scene}}, \hat{X} = ℰ([Q_{\text{scene}}; X])
$$

其中 $N = 16$ 是 scene query token 数量。相比原始 patch tokens（数百甚至上千），仅用 16 个 token 大幅压缩。
...
1. 多视角图像输入几何基础模型 $f_g$，生成 patch 级几何特征 $f_g(I) \in ℝ^{T \times M \times S \times D_g}$，其中 $D_g = 2048$

2. DINO backbone 输出的 $\hat{X}$ 通过几何投影器 $\phi$ 投影到同一空间：$\phi(\hat{X}) \in ℝ^{T \times M \times S \times D_g}$

3. **对齐损失**（cosine 相似度）：

$$
L_{\text{align}} = 1 - \cos\left(\text{LN}(\phi(\hat{X})), \text{LN}(f_g(I))\right)
$$
...
1. **Ego 状态编码器**（单层 MLP）：将驾驶命令、速度、加速度编码为 $S_{\text{ego}} \in ℝ^{T \times D_l}$

2. **Scene-Ego 融合**：多相机 scene tokens 聚合为全局感知表示，拼接 $S_{\text{ego}}$，得到统一的世界状态：

$$
S_{\text{world}} \in ℝ^{T \times (M \times N + 1) \times D_l}
$$
...
1. 随机初始化未来世界状态查询 $Q_{\text{future}} \in ℝ^{(T-1) \times (M \times N + 1) \times D_l}$
...
损失：$L_{\text{ego}} = L_{\text{cmd}} + L_v + L_a$

### 3.4 轨迹规划

**输入**：可学习的轨迹查询 $Q_\tau \in ℝ^{K \times n_p \times D_l}$ + 当前世界状态 $S_{\text{world}}^t$ + 当前驾驶命令 $C$
...
$$
L = L_{\text{traj}} + \alpha L_{\text{align}} + \beta L_{\text{wm}} + \gamma L_{\text{ego}}
$$

其中 $\alpha = 0.1, \beta = 0.2, \gamma = 0.1$。

- $L_{\text{traj}}$：L1 损失（模仿专家轨迹）
- $L_{\text{align}}$：cosine 相似度损失（几何蒸馏）
- $L_{\text{wm}}$：MSE 损失（未来世界状态预测）
- $L_{\text{ego}}$：交叉熵 + MSE（Ego 状态监督）
...
**Camera Prior 提取**：几何基础模型接收图像 + 内参 + 外参，输出空间-几何先验 $P$，封装相机 pose、深度估计和内参信息。

**Geometric Feature Encoding**：

$$
f_g(I) = \text{WorldMirror}(I, P; c)
$$

其中条件标志 $c = [1, 0, 1]$（使用相机 pose 和内参，不使用深度监督）。
